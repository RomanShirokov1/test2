...
class User {
    ...
    public static function user_info($d) {
        ...
        // info
        $q = DB::query("SELECT user_id, first_name, last_name, phone, email,  plot_id, last_login, access FROM users WHERE ".$where." LIMIT 1;") or die (DB::error());
    ...
    public static function user_edit_update($d = []) {
        ...
        $user_id = isset($d['user_id']) && is_numeric($d['user_id']) ? $d['user_id'] : 0;
        $validated = true;
        $fields_to_validate = ['first_name', 'last_name', 'email', 'phone'];
        foreach($d as $key=>$val) {
            if (in_array($key, $fields_to_validate) && empty($val))
                $validated = false;
        }
        if (!$validated) {
            return;
